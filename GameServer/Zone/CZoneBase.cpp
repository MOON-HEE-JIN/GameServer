...
void CZoneBase::Init(int ID, int ZoneID, int ProcID, int Maximum)
{
	m_iChannel = ID;
	m_iZoneID = ZoneID;
	m_iProcID = ProcID;
...
void CZoneBase::ZoneChangeJobProcess()
{
	ZONE_CHANGE_JOB job;
...
bool CZoneBase::Enqueue(ZONE_CHANGE_JOB& job)
{
	if (!m_bActive.load())
		return false;
...
bool CZoneBase::TryPush(CPlayer* pPlayer)
{
	if (pPlayer == nullptr)
	{
...
bool CZoneBase::EnterZone(CPlayer* pPlayer)
{
...
bool CZoneBase::LeaveZone(CPlayer* pPlayer)
{
